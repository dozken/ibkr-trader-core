...
class TestSendFillAlert(unittest.IsolatedAsyncioTestCase):
    ...
    @patch("ibkr_core.features.alerts.dispatcher.alert", new_callable=AsyncMock)
    ...
    @patch("ibkr_core.features.settings.service.load_settings", return_value={"alert_channels": []})
    @patch("ibkr_core.features.alerts.dispatcher.alert", new_callable=AsyncMock)
    ...
    @patch("ibkr_core.features.settings.service.load_settings", side_effect=Exception("settings error"))
    @patch("ibkr_core.features.alerts.dispatcher.alert", new_callable=AsyncMock)
...
class TestOnExecDetailsDispatchesAlert(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_fill_alert_task_is_created(self):
        ...
        with patch(
            "ibkr_core.features.trading.worker.IBKRWorker._db_update_fill_details"
        ), patch.object(
