...
class TestPeakNLVSeed(unittest.TestCase):
    ...
    def test_seeds_from_db_on_startup(self):
        from ibkr_core.features.portfolio.loops import _load_peak_nlv_from_db
        snap = MagicMock()
        ...
        with patch("ibkr_core.features.portfolio.loops.SessionLocal", return_value=mock_db):
    ...
    def test_returns_zero_on_db_error(self):
        from ibkr_core.features.portfolio.loops import _load_peak_nlv_from_db
        with patch("ibkr_core.features.portfolio.loops.SessionLocal", side_effect=Exception("db down")):
            result = _load_peak_nlv_from_db()
        self.assertEqual(result, 0.0)
...
class TestReconciliation(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_reconcile_marks_filled_when_position_exists(self):
        ...
        with patch("ibkr_core.features.trading.reconciliation.SessionLocal", return_value=mock_db):
    ...
    async def test_reconnect_triggers_reconciliation(self):
        ...
        with patch("ibkr_core.features.trading.reconciliation.reconcile_with_ibkr", new_callable=AsyncMock) as mock_recon, \
