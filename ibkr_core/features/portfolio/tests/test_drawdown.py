...
async def _run_one_tick(worker, health: dict, settings: dict, manage_drawdown: bool = True):
    ...
    with patch("ibkr_core.features.portfolio.loops._load_peak_nlv_from_db", return_value=health.get("peak_nlv", 0.0)), \
         patch("ibkr_core.features.portfolio.loops.asyncio.sleep", side_effect=_fake_sleep), \
         patch("ibkr_core.features.portfolio.loops.load_settings", return_value=settings), \
         patch("ibkr_core.features.portfolio.loops.send_alert", new_callable=AsyncMock) as mock_alert, \
...
class TestDailyLossLimit(unittest.TestCase):
    ...
    def _check(self, open_nlv: float, current_nlv: float, max_loss_pct: float = 5.0) -> bool:
        from ibkr_core.features.trading.loops import _exceeds_daily_loss_limit
