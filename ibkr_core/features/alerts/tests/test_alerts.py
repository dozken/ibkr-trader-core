...
class TestDispatcher(unittest.IsolatedAsyncioTestCase):
    async def test_no_channels_no_call(self):
        ...
        with patch("ibkr_core.features.alerts.dispatcher.send_telegram") as mock_tg:
            await alert("title", "body", channels=[])
        mock_tg.assert_not_called()

    async def test_telegram_channel_calls_send_telegram(self):
        from ibkr_core.features.alerts.dispatcher import alert
        with patch("ibkr_core.features.alerts.dispatcher.send_telegram", new_callable=AsyncMock) as mock_tg:
