...
class TestSendMessage(unittest.IsolatedAsyncioTestCase):
    async def test_posts_to_telegram(self):
        from ibkr_core.features.alerts.telegram_bot import _send_message
        mock_client = AsyncMock()
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        mock_client.__aexit__ = AsyncMock(return_value=False)
        mock_client.post = AsyncMock()
        with patch("ibkr_core.features.alerts.telegram_bot.httpx.AsyncClient", return_value=mock_client):
...
class TestAnswerCallback(unittest.IsolatedAsyncioTestCase):
    async def test_posts_answer_callback(self):
        from ibkr_core.features.alerts.telegram_bot import _answer_callback
        mock_client = AsyncMock()
        mock_client.__aenter__ = AsyncMock(return_value=mock_client)
        mock_client.__aexit__ = AsyncMock(return_value=False)
        mock_client.post = AsyncMock()
...
class TestHandleCommand(unittest.IsolatedAsyncioTestCase):
    ...
    async def _run(self, command, args=None, worker=None):
        from ibkr_core.features.alerts.telegram_bot import _handle_command
        if worker is None:
            worker = _make_worker()
        send_mock = AsyncMock()
    ...
    async def test_signals_no_actionable(self):
        ...
        with patch("ibkr_core.core.strategy.registry.get_active_strategy", return_value=fake_strategy), \
             patch("ibkr_core.core.strategy.get_active_strategy", return_value=fake_strategy):
    ...
    async def test_signals_actionable_shown(self):
        ...
        with patch("ibkr_core.core.strategy.registry.get_active_strategy", return_value=fake_strategy), \
             patch("ibkr_core.core.strategy.get_active_strategy", return_value=fake_strategy):
            with patch("ibkr_core.features.compliance.screening.async_shariah_screen", AsyncMock()) as mock_screen:
                mock_comp = MagicMock()
    ...
    async def test_liquidate_success(self):
        ...
        with patch("ibkr_core.features.trading.trader.Trader") as MockTrader:
...
class TestTelegramBotLoop(unittest.IsolatedAsyncioTestCase):
    ...
    async def _run_one_poll(self, updates, env=None):
        ...
        with patch.dict(os.environ, env):
            with patch("ibkr_core.features.alerts.telegram_bot.httpx.AsyncClient", return_value=mock_client):
                with patch("ibkr_core.features.alerts.telegram_bot._handle_command", AsyncMock()) as mock_cmd:
                    with patch("ibkr_core.features.alerts.telegram_bot.asyncio.sleep", AsyncMock()):
                        try:
                            await telegram_bot_loop(_make_worker(), health)
