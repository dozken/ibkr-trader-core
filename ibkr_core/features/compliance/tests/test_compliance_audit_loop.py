...
class TestVixTierNotification(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_first_run_no_alert(self):
        ...
        from ibkr_core.features.compliance.loops import _check_vix_tier_change
        health = self._health(vix_tier=None)
        with patch("ibkr_core.features.compliance.loops.send_alert", new_callable=AsyncMock) as mock_alert:
    ...
    async def test_health_dict_updated_with_new_tier(self):
        from ibkr_core.features.compliance.loops import _check_vix_tier_change
        health = self._health(vix_tier="CALM")
        with patch("ibkr_core.features.compliance.loops.send_alert", new_callable=AsyncMock):
