...
from ibkr_core.core.models import Base, AuditLog
from ibkr_core.core.audit import verify_audit_chain, secure_log_entry
...
class TestAuditVerifyEndpoint(unittest.TestCase):
    ...
    def _call(self, valid: bool, count: int):
        from ibkr_core.features.compliance.router import verify_audit_integrity
        mock_db = MagicMock()
        mock_db.query.return_value.scalar.return_value = count
        with patch("ibkr_core.features.compliance.router.verify_audit_chain", return_value=valid):
