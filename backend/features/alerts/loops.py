...
from backend.core.market_hours import is_market_open
from backend.core.models import PositionCompliance, PortfolioSnapshot
...
async def daily_report_loop(worker, health: dict) -> None:
        ...
        try:
            ...
            if not is_market_open("NMS") and now.hour >= 16 and today != last_reported_date:
                if worker.ib.isConnected():
                    logger.info("Market closed. Generating daily digest...")
                    ...
                    try:
                        ...
                        logger.warning("Benchmark comparison failed: %s", bench_err)

                    channels = load_settings().get("alert_channels", [])
                    await send_alert("Daily Performance Digest", body, channels)
