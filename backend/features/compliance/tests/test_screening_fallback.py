...
class TestScreeningFallbackZoyaHit(unittest.TestCase):
    ...
    def test_zoya_compliant_returns_compliant(self):
        with patch("backend.features.compliance.screening.fetch_shariah_verdict",
                   return_value=_zoya_verdict(compliant=True)), \
             patch("backend.features.compliance.screening.fetch_financial_data",
                   return_value=_financial_data()), \
             patch("backend.features.compliance.screening._load_settings",
