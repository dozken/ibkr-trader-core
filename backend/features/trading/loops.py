...
from backend.core.market_hours import market_status, is_in_trading_window
from backend.core.health_utils import set_loop_error, clear_loop_error
from backend.core.websocket import ConnectionManager, WSBaseMessage
from backend.features.alerts.dispatcher import alert as send_alert
from backend.core.strategy import get_active_strategy, MarketContext

# AI analysis websocket message — private fork ships richer payloads.
try:
    from backend.features.ai.schemas import AIAnalysisMessage, AIAnalysisPayload  # type: ignore
except ImportError:
...
from backend.features.compliance.schemas import (
    ComplianceResultMessage,
    ComplianceResultPayload,
    ComplianceStatus,
)
from backend.features.compliance.screening import async_shariah_screen, screen_many
from backend.features.portfolio.allocator import PortfolioAllocator
from backend.features.settings.service import load_settings
...
from backend.features.trading.trader import Trader, _estimate_fee
from backend.core.database import SessionLocal
from backend.core.models import PendingSignal, TradeHistory
    ...
    while True:
        try:
            ...
            if over > 0:
                ...
                from backend.core.models import SignalLog
                scored = []
                for pos in positions:
                    sym = pos.get("symbol", "")
            ...
            if not can_buy:
                ...
                if buy_signals:
                    ...
                    try:
                        from backend.features.ai.strategy import _portfolio_sector_weights  # type: ignore
...
import os
import json
from backend.features.compliance.screening import async_shariah_screen
...
async def discovery_loop(worker, manager: ConnectionManager, health: dict, account_id: Optional[int] = None) -> None:
    ...
    while True:
        try:
            ...
            from backend.core.market_hours import any_market_open
...
async def position_rerating_loop(worker, manager: ConnectionManager, health: dict, account_id: Optional[int] = None) -> None:
    ...
    while True:
        try:
            ...
            try:
                from backend.features.ai.strategy import get_multi_factor_score  # type: ignore
            except ImportError:
                logger.info("position_rerating_loop: AI scoring module not present — skipping rerate cycle")
                await asyncio.sleep(4 * 3600)
                continue
            from backend.features.compliance.vix import get_current_vix, vix_to_ratio_buffer
            ...
            for pos in positions:
                ...
                try:
                    ...
                    if score <= threshold:
                        ...
                        exchange = cr.exchange or "NMS"
                        from backend.core.market_hours import market_status
                        if market_status(exchange)["is_open"]:
                            signal = TradeSignal(
                                symbol=symbol, action="SELL", confidence=80,
                                sentiment_score=0.0,
