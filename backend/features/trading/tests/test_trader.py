...
from backend.features.trading.trader import Trader, _calculate_position_size, _stop_take_prices, _MIN_FRACTIONAL_QTY, _exceeds_concentration_limit
...
class TestStopTakePrices(unittest.TestCase):
    ...
    def test_atr_stops_used_when_enabled(self):
        ...
        with patch("backend.features.trading.trader._calculate_atr", return_value=5.0):
...
class TestTrader(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.patcher_db = patch('backend.features.trading.trader.SessionLocal')
        self.mock_session_local = self.patcher_db.start()
        self.mock_db = MagicMock()
        self.mock_session_local.return_value = self.mock_db
        self.patcher_settings = patch('backend.features.trading.trader._load_settings',
    ...
    @patch('backend.features.trading.trader.check_shariah_compliance')
    async def test_execute_buy_dry_run_state(self, mock_check):
        ...
        with patch('backend.features.trading.trader._load_settings', return_value={**_SETTINGS, "dry_run": True}):
    ...
    async def test_pre_screened_skips_check_shariah_compliance(self):
        ...
        trade_req = TradeCreate(symbol="AAPL", quantity=10, side="BUY")
        with patch("backend.features.trading.trader.check_shariah_compliance") as mock_check:
            trade = await self.trader.execute_trade(trade_req, pre_screened=_COMPLIANT)
...
class TestVIXAdjustedSizing(unittest.TestCase):
    ...
    def _size(self, vix_tier: str) -> float:
        from backend.features.trading.trader import _calculate_position_size
        tier_map = {"CALM": 15.0, "ELEVATED": 27.0, "CRISIS": 42.0}
        vix_val = tier_map[vix_tier]
        with patch("backend.features.trading.trader.get_current_vix", return_value=vix_val):
    ...
    def test_vix_fetch_failure_falls_back_to_full_size(self):
        with patch("backend.features.trading.trader.get_current_vix", side_effect=Exception("network error")):
...
class TestSectorConcentrationLimit(unittest.TestCase):
    ...
    def test_no_sector_data_passes(self):
        ...
        with patch("backend.features.trading.trader.SessionLocal", return_value=mock_db):
...
class TestIsInTradingWindow(unittest.TestCase):
    ...
    def _check(self, hour: int, minute: int, start_off: int = 30, end_off: int = 30) -> bool:
        from backend.core.market_hours import is_in_trading_window
        from datetime import timezone
        tz = __import__("zoneinfo").ZoneInfo("America/New_York")
        fake_now = datetime(2024, 1, 15, hour, minute, 0, tzinfo=tz)  # Monday
        with patch("backend.core.market_hours.datetime") as mock_dt:
