...
from backend.core.models import Base, Account
from backend.features.trading.account_manager import AccountManager
...
def test_get_worker_creates_worker_on_first_call():
    mgr = AccountManager()
    with patch("backend.features.trading.account_manager.IBKRWorker") as MockWorker:
...
def test_list_account_ids_after_get_worker():
    mgr = AccountManager()
    with patch("backend.features.trading.account_manager.IBKRWorker"):
...
def test_remove_worker_disconnects_and_removes():
    ...
    with patch("backend.features.trading.account_manager.IBKRWorker", return_value=mock_worker):
        mgr.get_worker(account_id=1, host="127.0.0.1", port=7497, client_id=1)
...
def test_from_db_loads_active_accounts(db):
    ...
    with patch("backend.features.trading.account_manager.IBKRWorker") as MockWorker:
        MockWorker.side_effect = [MagicMock(), MagicMock()]
        with patch("backend.features.trading.account_manager.SessionLocal", return_value=db):
...
async def test_connect_all_calls_connect_on_each_worker():
    ...
    with patch("backend.features.trading.account_manager.IBKRWorker", side_effect=[w1, w2]):
        mgr.get_worker(account_id=1, host="127.0.0.1", port=7497, client_id=1)
        mgr.get_worker(account_id=2, host="127.0.0.1", port=7496, client_id=2)
...
async def test_connect_all_returns_false_on_failure():
    ...
    with patch("backend.features.trading.account_manager.IBKRWorker", return_value=w1):
        mgr.get_worker(account_id=1, host="127.0.0.1", port=7497, client_id=1)
