...
class TestDiscoveryLoopDisabled(unittest.IsolatedAsyncioTestCase):
    async def test_loop_skips_when_disabled(self):
        ...
        with patch("backend.features.trading.loops.load_settings", return_value=_SETTINGS_DISABLED), \
             patch("asyncio.sleep", side_effect=_fake_sleep), \
             patch("backend.features.trading.loops.discover_halal_buys", new_callable=AsyncMock) as mock_discover:
            from backend.features.trading.loops import discovery_loop
            manager = MagicMock()
            try:
...
class TestDiscoveryLoopEnabled(unittest.IsolatedAsyncioTestCase):
    async def test_loop_dispatches_compliant_signal_above_threshold(self):
        ...
        with patch("backend.features.trading.loops.load_settings", return_value=_SETTINGS_ENABLED), \
             patch("backend.features.trading.loops.discover_halal_buys", new_callable=AsyncMock, return_value=[signal]), \
             patch("backend.features.trading.loops.screen_many", new_callable=AsyncMock, return_value=[compliance]), \
             patch("backend.features.trading.loops.market_status", return_value={"is_open": True}), \
             patch("backend.features.trading.loops.Trader", return_value=mock_trader), \
             patch("backend.features.trading.loops.send_alert", new_callable=AsyncMock), \
             patch("asyncio.sleep", side_effect=_fake_sleep):
            from backend.features.trading.loops import discovery_loop
            manager = MagicMock()
    ...
    async def test_loop_skips_when_market_closed(self):
        ...
        with patch("backend.features.trading.loops.load_settings", return_value=_SETTINGS_ENABLED), \
             patch("backend.features.trading.loops.discover_halal_buys", new_callable=AsyncMock, return_value=[signal]), \
             patch("backend.features.trading.loops.screen_many", new_callable=AsyncMock, return_value=[compliance]), \
             patch("backend.features.trading.loops.market_status", return_value={"is_open": False}), \
             patch("backend.features.trading.loops.Trader", return_value=mock_trader), \
             patch("asyncio.sleep", side_effect=_fake_sleep):
            from backend.features.trading.loops import discovery_loop
            manager = MagicMock()
    ...
    async def test_loop_skips_non_compliant_signal(self):
        ...
        with patch("backend.features.trading.loops.load_settings", return_value=_SETTINGS_ENABLED), \
             patch("backend.features.trading.loops.discover_halal_buys", new_callable=AsyncMock, return_value=[signal]), \
             patch("backend.features.trading.loops.screen_many", new_callable=AsyncMock, return_value=[bad_compliance]), \
             patch("backend.features.trading.loops.market_status", return_value={"is_open": True}), \
             patch("backend.features.trading.loops.Trader", return_value=mock_trader), \
             patch("asyncio.sleep", side_effect=_fake_sleep):
            from backend.features.trading.loops import discovery_loop
    ...
    async def test_health_updated_after_scan(self):
        ...
        with patch("backend.features.trading.loops.load_settings", return_value=_SETTINGS_ENABLED), \
             patch("backend.features.trading.loops.discover_halal_buys", new_callable=AsyncMock, return_value=[]), \
             patch("backend.features.trading.loops.screen_many", new_callable=AsyncMock, return_value=[]), \
             patch("backend.features.trading.loops.market_status", return_value={"is_open": True}), \
             patch("backend.features.trading.loops.Trader"), \
             patch("asyncio.sleep", side_effect=_fake_sleep):
            from backend.features.trading.loops import discovery_loop
            manager = MagicMock()
            manager.broadcast = AsyncMock()
