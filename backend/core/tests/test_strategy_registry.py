...
from backend.core.strategy import Strategy, load_strategy, get_active_strategy
from backend.core.strategy.registry import DEFAULT_STRATEGY
...
def test_rejects_missing_colon():
    with pytest.raises(ImportError, match="module.path:ClassName"):
        load_strategy("backend.strategies.sma_crossover.SMACrossover")
...
def test_rejects_missing_class():
    with pytest.raises(ImportError, match="not found"):
        load_strategy("backend.strategies.sma_crossover:DoesNotExist")


def test_rejects_non_strategy_class():
    with pytest.raises(ImportError, match="does not subclass"):
        load_strategy("backend.core.strategy.base:MarketContext")
